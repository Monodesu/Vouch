...
### 库存转移
- 将某账号的可交易物品发送到已配置的交易链接 —— **全部物品** 或 **手动挑选** 的一部分
  （带物品图片），每款游戏一份报价
- 预设游戏（CS2、TF2、Dota 2、Rust、Steam）以及 **自定义 appid**
- 自动在移动端确认生成的报价

### 身份验证器管理
- **添加身份验证器** 向导 —— **无需手机号** 也能使用（Steam 会通过邮件发送最终确认码）；
  撤销码会显示出来，并在单独的对话框中再次确认
