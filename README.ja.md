...
取引オファーの確認と応答、インベントリの転送、認証アプリのリンク・リンク解除まで、
すべてを 1 つのアプリから行えます。
...
### アカウント
- **検索**、**複数選択**、**ドラッグでの並べ替え** に対応したマルチアカウントサイドバー
...
- ユーザー名 / パスワードのコピー、保存済みパスワードのインライン編集
...
### インベントリ転送
- アカウントの取引可能なアイテムを、設定済みの取引リンクへ送信 — **すべてのアイテム** または
  **手動で選んだ** 選択範囲 (アイテム画像付き)、ゲームごとに 1 オファー
- プリセットゲーム (CS2, TF2, Dota 2, Rust, Steam) に加えて **カスタム appid**
- 結果として生成されるオファーをモバイルで自動確認

### 認証アプリの管理
- **認証アプリの追加** ウィザード — **電話番号なし** で動作 (Steam が finalize コードをメールで送信)、
  失効コードは別のダイアログで表示され再確認されます
